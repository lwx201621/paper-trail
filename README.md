...
- 分析投稿接收率、首轮决定周期、接收周期、作者身份和出版社分布
- 自动识别修回逾期、临近截止、长期未更新、缺少链接和缺少 JCR
- 会议投稿、通知、定稿和会期追踪，并以本地世界地图显示会议足迹
- 支持编辑论文和当前投稿信息，并将修回截止日期导出为 `.ics` 日历
- JSON 完整备份/恢复、AES-256-GCM 加密备份、CSV 表格导出、JCR CSV 批量导入
...
会议模块可记录截稿、结果通知、定稿和举办日期，并在本地世界地图中显示会址。常用学术城市可直接选择，也可以手动填写经纬度。世界边界来自 `world-atlas` / Natural Earth，地图渲染和会议信息处理均在浏览器本地完成。

## 隐私提醒

- 同一网址在不同浏览器或不同设备上拥有独立数据。
- 清理浏览器网站数据会删除记录，请定期导出 JSON 备份。
- 加密备份使用 PBKDF2-SHA256 和 AES-256-GCM 在浏览器本地处理；请妥善保存密码，密码遗失后无法恢复。
